import { X, DollarSign, Trash2, MessageSquare, Star, Flame, ShoppingCart, Percent, Shield, MessageCircle } from 'lucide-react'
...
export function ModalDetalhesProduto({ isOpen, onClose, produto }: ModalProps) {
  ...
  useEffect(() => {
    if (isOpen && produto?.id) {
      ...
      getProductComments(produto.id).then(data => {
        setComentarios(data)
      }).finally(() => {
      ...
      >
        ...
        >
          ...
          <div className="flex justify-between items-center p-6 md:p-8 pb-4 shrink-0 border-b border-slate-100 bg-[#F8F9FC]">
            <h2 className="text-2xl md:text-[32px] font-black text-[#0A113F] tracking-tight">Detalhes do Produto</h2>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
              ...
              <div className="lg:col-span-5 bg-white border border-slate-200/80 rounded-[32px] p-6 shadow-sm flex flex-col justify-between min-h-[450px]">
                <div>
                  ...
                  <div className="w-full aspect-[16/9] bg-[#E1E6EB] rounded-2xl relative border border-slate-300/60 flex items-center justify-center overflow-hidden mb-5 shrink-0">
                    {/* Linhas cruzadas simulando o placeholder da imagem (só aparecem se não houver imagem) */}
                    {!produto.imagem && (
                    ...
                    <div className="flex justify-between items-center pt-4 border-t border-slate-100">
                      <div className="flex items-center gap-2 text-slate-700">
                        ...
                        <div className="leading-tight">
                          <p className="text-xs font-black">{produto.total_tickets || '0'} Tickets</p>
              ...
              <div className="lg:col-span-7 bg-white border border-slate-200/80 rounded-[32px] p-6 md:p-8 shadow-sm flex flex-col min-h-[450px]">
                ...
                <div className="mb-6 shrink-0">
                  <h4 className="text-[11px] font-black text-[#0061FF] tracking-wider uppercase mb-2">Descrição do Produto</h4>
                ...
                <div className="flex flex-col flex-1 min-h-[200px]">
                  <h4 className="text-[11px] font-black text-[#0061FF] tracking-wider uppercase mb-3 flex items-center gap-2 shrink-0">
                      <MessageCircle className="w-4 h-4" /> Comentários sobre o Produto
                  </h4>

                  {/* Área Interna de Comentários com Scroll Próprio se houver muitos */}
                  <div className="flex-1 bg-slate-50 border border-slate-100 rounded-2xl p-4 overflow-y-auto max-h-[260px] scrollbar-hide flex flex-col gap-3 mb-6">
                      {isLoadingComments ? (
                          ...
                          <div className="flex flex-col gap-3 h-full">
                              {comentarios.map((comentario, index) => (
                                  <div key={index} className="bg-white p-4 rounded-xl text-sm font-medium text-slate-600 border border-slate-200 shadow-sm w-full shrink-0">
                                      "{comentario}"
                                  </div>
                              ))}
                          </div>
                  ...
                  {/* Métricas Dinâmicas Movidas para dentro da seção de comentários */}
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-auto shrink-0 w-full">
                    ...
                    <div className="bg-white border border-slate-200/60 rounded-2xl p-4 flex flex-col justify-between shadow-sm min-h-[100px] w-full">
                      <div className="flex items-center gap-2 mb-2">
                        <div className="p-2 rounded-xl bg-[#E6F4FE] text-[#0091FF] shrink-0"><ShoppingCart className="w-4 h-4" /></div>
                        <div className="text-[10px] font-bold text-slate-400 leading-tight">Total<br/>Vendido</div>
                      </div>
                      <div className="text-base font-black text-[#0A113F] break-words">{produto.vendidos || '0'}</div>
                    </div>

                    {/* Receita Total */}
                    <div className="bg-white border border-slate-200/60 rounded-2xl p-4 flex flex-col justify-between shadow-sm min-h-[100px] w-full">
                      <div className="flex items-center gap-2 mb-2">
                        <div className="p-2 rounded-xl bg-[#E6F4FE] text-[#0091FF] shrink-0"><DollarSign className="w-4 h-4" /></div>
                        <div className="text-[10px] font-bold text-slate-400 leading-tight">Receita<br/>total</div>
                      </div>
                      <div className="text-base font-black text-[#0A113F] break-words">{produto.receitaTotal || 'R$ 0,00'}</div>
                    </div>

                    {/* Estoque Total */}
                    <div className="bg-white border border-slate-200/60 rounded-2xl p-4 flex flex-col justify-between shadow-sm min-h-[100px] w-full">
                      <div className="flex items-center gap-2 mb-2">
                        <div className="p-2 rounded-xl bg-[#E6F4FE] text-[#0091FF] shrink-0"><Percent className="w-4 h-4" /></div>
                        <div className="text-[10px] font-bold text-slate-400 leading-tight">Estoque<br/>total</div>
                      </div>
                      <div className="text-base font-black text-[#0A113F] break-words">{produto.estoque || '0'}</div>
                    </div>

                    {/* Avaliação Média */}
                    <div className="bg-white border border-slate-200/60 rounded-2xl p-4 flex flex-col justify-between shadow-sm min-h-[100px] w-full">
                      <div className="flex items-center gap-2 mb-2">
                        <div className="p-2 rounded-xl bg-[#E6F4FE] text-[#0091FF] shrink-0"><Shield className="w-4 h-4" /></div>
                        <div className="text-[10px] font-bold text-slate-400 leading-tight">Avaliação<br/>média</div>
                      </div>
                      <div className="text-base font-black text-[#0A113F] break-words">{produto.avaliacao || '0.0'}</div>
                    </div>
