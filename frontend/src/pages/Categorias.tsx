...
interface ModalRevisoesProps {
  categoria: Categoria;
  onClose: () => void;
}

function ModalRevisoes({ categoria, onClose }: ModalRevisoesProps) {
  ...
  useEffect(() => {
    async function carregar() {
      ...
      setErro(null);
      try {
        const data = await getProdutosPorRevisao(categoria.nome_categoria);
        setProdutos(data);
      } catch {
  ...
  return (
    ...
    >
      <div
        className="bg-card rounded-3xl shadow-xl border border-border w-full max-w-3xl flex flex-col max-h-[80vh]"
      ...
      >
        ...
        <div className="flex items-center justify-between p-8 pb-4 shrink-0">
          ...
          <button
            onClick={onClose}
            className="w-9 h-9 rounded-full bg-background flex items-center justify-center text-muted-foreground hover:text-foreground transition-colors border border-border shrink-0"
          >
            ✕
          </button>
        </div>
              ...
              <table className="w-full border-separate border-spacing-y-0">
                <thead>
                  <tr className="bg-[#020854] text-white">
                    <th className="py-3 px-5 text-left rounded-l-xl text-[10px] font-black uppercase tracking-widest border-none">
                      ID Produto
                    </th>
                    <th className="py-3 px-5 text-left text-[10px] font-black uppercase tracking-widest border-none">
                      Nome
                    </th>
                    <th className="py-3 px-5 text-left text-[10px] font-black uppercase tracking-widest border-none">
                      SKU
                    ...
                    <th className="py-3 px-5 text-center text-[10px] font-black uppercase tracking-widest border-none">
                      Estoque
                    </th>
                    <th className="py-3 px-5 text-center text-[10px] font-black uppercase tracking-widest border-none">
                      Avaliação
                    </th>
                    <th className="py-3 px-5 text-center rounded-r-xl text-[10px] font-black uppercase tracking-widest border-none">
                      Status
                    </th>
                  </tr>
                </thead>
                <tbody>
                  {produtos.map((prod, idx) => (
                    ...
                    >
                      <td className="py-3 px-5 border-0 rounded-l-xl">
                        <span className="font-black text-[#020854] dark:text-foreground text-sm">
                          {prod.id_produto.replace(/^#/, "")}
                        </span>
                      </td>
                      ...
                      <td className="py-3 px-5 border-0">
                        <span className="font-bold text-foreground text-sm">
                          {formatBRL(prod.preco)}
                        </span>
                      </td>
                      <td className="py-3 px-5 border-0 text-center">
                        {prod.estoque_disponivel === 0 ? (
                          <span className="px-2 py-1 rounded-full text-[10px] font-black uppercase border border-[#FECACA] bg-[#FEE2E2] text-[#B91C1C]">
                            Esgotado
                          </span>
                        ) : (
                          <span
                            className={`px-2 py-1 rounded-full text-[10px] font-black uppercase border ${
                              prod.estoque_disponivel <= 10
                                ? "border-amber-200 bg-amber-50 text-amber-700"
                                : "border-sky-200 bg-sky-100 text-sky-700"
                            }`}
                          >
                            {prod.estoque_disponivel} un
                          </span>
                        )}
                      </td>
                      <td className="py-3 px-5 border-0 text-center">
                        {prod.media_nota_produto != null ? (
                          <div className="flex items-center justify-center gap-1">
                            <span className="font-black text-foreground text-sm">
                              {prod.media_nota_produto.toFixed(1)}
                            </span>
                            <span className="text-[#FFD700] text-base leading-none">
                              ★
                            </span>
                          </div>
                        ) : (
                      ...
                      <td className="py-3 px-5 border-0 text-center rounded-r-xl">
                        {prod.ativo === "Não" ? (
                          <span className="px-2 py-1 rounded-full text-[10px] font-black uppercase border border-slate-200 bg-slate-100 text-slate-500">
                            Inativo
                          </span>
                        ) : prod.estoque_disponivel <= 10 ? (
                          <span className="px-2 py-1 rounded-full text-[10px] font-black uppercase border border-amber-200 bg-amber-50 text-amber-700">
                            Baixo Estoque
                          </span>
                        ) : (
                          <span className="px-2 py-1 rounded-full text-[10px] font-black uppercase border border-[#BBF7D0] bg-[#DCFCE7] text-[#15803D]">
                            Ativo
                          </span>
                        )}
                      </td>
