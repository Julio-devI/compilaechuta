      ...
      {isOpen && (
        ...
        >
          ...
          <motion.div
            ...
            className="bg-white w-full max-w-6xl rounded-[32px] overflow-hidden shadow-2xl relative max-h-[90vh] overflow-y-auto scrollbar-hide"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Header */}
            <div className="flex justify-between items-center p-8 pb-4">
              <h2 className="text-3xl font-black text-[#020854]">Detalhes do pedido</h2>
            ...
            <div className="p-4 pt-0 grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <div className="space-y-6">
                ...
                {/* Card Info Principal */}
                <div className="border border-slate-200 rounded-3xl p-6 relative">
                  ...
                  <h3 className="text-3xl font-black text-[#020854] mt-1">{pedido.id}</h3>
                  <p className="text-slate-400 text-sm font-medium">Comprado em {pedido.data}, 14:32:00</p>

                  <div className="grid grid-cols-3 gap-4 mt-6">
                    <div className="bg-[#F8FAFC] p-4 rounded-2xl border border-slate-100">
                      <span className="flex items-center gap-1.5 text-[10px] font-bold text-slate-500 mb-1">
                        <Clock className="w-3 h-3" /> Tempo aberto
                      </span>
                      <span className="text-lg font-bold text-slate-700">{pedido.tempoAberto.split(' ')[0]} dias</span>
                    </div>
                    ...
                    <div className="bg-[#F8FAFC] p-4 rounded-2xl border border-slate-100">
                      ...
                      <span className="text-sm font-bold text-slate-700 block">Loggi Express</span>
                      <span className="text-[10px] text-slate-400 uppercase font-medium">LE9982KX-BR</span>
                    </div>
                  </div>

                  {/* Pipeline Logístico */}
                  <div className="mt-8">
                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block mb-4">Pipeline logístico</span>
                        ...
                        <div key={i} className="flex flex-col items-center flex-1 relative">
                          <div className={`w-10 h-10 rounded-full flex items-center justify-center z-10
                            ${step.active === true ? 'bg-[#020854] text-white' :
                              step.active === 'current' ? 'bg-red-500 text-white' : 'bg-slate-100 text-slate-300'}`}>
                          ...
                          {i < arr.length - 1 && (
                            <div className={`absolute h-[2px] w-full top-5 left-1/2 -z-0 ${step.active === true ? 'bg-[#020854]' : 'bg-slate-100'}`} />
                          )}
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="bg-white rounded-2xl p-4 flex items-center justify-between shadow-sm">
                    <div>
                      <div className="flex gap-2 mb-1">
                        <span className="font-black text-slate-700">TK-77821</span>
                      ...
                      <p className="text-xs text-slate-500 font-medium">Cliente solicita previsão atualizada da entrega</p>
                    </div>
              ...
              <div className="space-y-6">
                {/* Contexto do Cliente */}
                <div className="border border-[#ADE9FF] rounded-3xl p-6">
                  <div className="flex justify-between items-start mb-4 ">
                    <div>
                      <span className="text-[10px] font-bold text-blue-500 uppercase">Contexto do cliente</span>
                      <h4 className="text-lg font-black text-[#020854]">Quem comprou</h4>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        <span className="font-black text-[#020854] text-xl">{pedido.cliente}</span>
                      ...
                      <div className="flex gap-6 mt-2">
                        ...
                        <div className="flex flex-col gap-1 min-w-0 flex-1">
                          ...
                          <span className="text-slate-400 text-xs font-bold">38 pedidos no total</span>
                        </div>
                ...
                {/* Itens do Pedido */}
                <div className="border border-slate-200 rounded-3xl p-6 flex-1">
                  <span className="text-[10px] font-bold text-blue-500 uppercase">Itens do pedido</span>
                  <h4 className="text-lg font-black text-[#020854] mb-6">Produtos & Performance</h4>
                      ...
                      <div key={i} className="flex items-center gap-4 group">
                        ...
                        <div className="flex-1">
                          <div className="flex justify-between">
                            <span className="font-bold text-[#020854] text-sm">{item.name}</span>
                            <span className="font-black text-[#020854] text-sm">R$ {item.price}</span>
                          </div>
                  ...
                  <div className="mt-8 pt-6 border-t border-dashed border-slate-200 flex justify-between items-end">
                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Total do pedido</span>
                    <span className="text-3xl font-black text-[#020854]">{pedido.valor}</span>
                  </div>
                </div>
              </div>
