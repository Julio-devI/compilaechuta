...
def validate_empty_input(question: str) -> None:
    ...
    """
    if question.strip() == "":
        raise GuardrailError("Input do usuario esta vazio.", error_code="EMPTY_INPUT")
...
def validate_destructive_queries(sql: str) -> None:
    ...
    try:
        ...
        raise GuardrailError(f"Falha ao parsear SQL: {exc}", error_code="SQL_PARSE_ERROR") from exc
    if not isinstance(parsed, exp.Select):
        raise GuardrailError(
            f"Apenas consultas SELECT são permitidas. "
            f"Tipo detectado: {type(parsed).__name__}",
            error_code="DESTRUCTIVE_QUERY"
        )
    ...
    for table in parsed.find_all(exp.Table):
        ...
        if table.name not in allowlist:
            raise GuardrailError(
                ...
                error_code="SCHEMA_VIOLATION_ALLOWLIST"
            )
    ...
    for col in parsed.find_all(exp.Column):
        ...
        if table_ref:
            ...
            raise GuardrailError(
                ...
                error_code="SCHEMA_VIOLATION_SEMANTIC"
            )
        else:
            found = False
